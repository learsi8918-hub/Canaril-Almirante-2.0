...
*   **🧪 Suite de Testes e Qualidade de Código:** Cobertura rigorosa de testes unitários (`flutter_test`) integrada ao pipeline. Valida algoritmos de cálculo de fertilidade, impede erros de divisão por zero e garante a precisão de 13 dias do ciclo biológico antes de autorizar a compilação do instalador.
*   **📦 Compilação Automatizada de Binários (Nuvem APK):** Pipeline de Integração Contínua (CI/CD) operando sob o motor v4 de upload do GitHub Actions. O sistema executa testes ornitológicos em ambiente virtualizado isolado e gera o instalador móvel nativo (**APK**) disponível através do painel de Artefatos para testes em smartphones Android.
...
│   └── build_app.yml             # Script v4 de automação de testes e geração de APK nas Actions
